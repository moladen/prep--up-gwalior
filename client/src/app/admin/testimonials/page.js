...
export default function TestimonialsPage() {
  ...
  const fetchTestimonials = useCallback(
    async (page = 1, q = search) => {
      ...
      try {
        ...
        const data = await api.getTestimonials(params.toString());
  ...
  useEffect(() => {
    const timer = setTimeout(() => fetchTestimonials(1), 300);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing) {
        await api.updateTestimonial(editing._id, fd);
        showToast("Testimonial updated successfully");
      } else {
        await api.createTestimonial(fd);
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api.deleteTestimonial(deleteTarget._id);
    ...
    <div className="space-y-6">
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        {loading ? (
          ...
          <EmptyState title="No testimonials found" description="Add student testimonials to build trust." />
        ) : (
            ...
            <table className="w-full min-w-[680px] text-left text-sm">
              <thead className="border-b border-slate-200 bg-slate-50">
                <tr>
                  ...
                  <th className="px-5 py-3 font-semibold text-slate-700">Rating</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {testimonials.map((item) => (
                  <tr key={item._id} className="hover:bg-slate-50">
                    ...
                    <td className="px-5 py-3 text-amber-500">{"★".repeat(item.rating || 5)}</td>
                    <td className="px-5 py-3">
                      <span
        ...
        {!loading && testimonials.length > 0 && (
          <div className="border-t border-slate-200 p-4">
            <Pagination
              ...
              onPageChange={(p) => fetchTestimonials(p)}
            />
          </div>
        )}
      </div>
